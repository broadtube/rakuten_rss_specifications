...
### Excel ワークシート関数
```excel
RssMarginOpenOrder(発注ID, 発注トリガー, 銘柄コード, 売買区分, 注文区分, SOR区分, 信用区分, 注文数量, 価格区分, 注文価格, 執行条件, 注文期日, 口座区分, 逆指値条件価格, 逆指値条件区分, 逆指値価格区分, 逆指値価格, セット注文区分, セット注文価格区分, セット注文価格, セット注文執行条件, セット注文期日)
```

### VBA関数
```vb
RssMarginOpenOrder_V(発注ID, 銘柄コード, 売買区分, 注文区分, SOR区分, 信用区分, 注文数量, 価格区分, 注文価格, 執行条件, 注文期日, 口座区分, 逆指値条件価格, 逆指値条件区分, 逆指値価格区分, 逆指値価格, セット注文区分, セット注文価格区分, セット注文価格, セット注文執行条件, セット注文期日)
```
...
| 2 | 発注トリガー | ● | ● | ● | number | 1(True):発注, 0(False):待機<br>省略時は0(False):待機 |
| 3 | 銘柄コード | ● | ● | ● | string | 4-5桁英数字の銘柄コード<br>「.市場」は省略可（.T:.東証, .JAX:JASDAQ, .JNX:JNX） |
| 4 | 売買区分 | ● | ● | ● | number | 1:売建, 3:買建 |
| 5 | 注文区分 | ● | ● | ● | number | 0:通常注文, 1:逆指値付通常注文, 2:逆指値注文 |
| 6 | SOR区分 | ● | ● | ● | number | 0:通常注文, 1:SOR注文 |
| 7 | 信用区分 | ● | ● | ● | number | 1:制度信用(6ヶ月), 2:一般信用(無制限), 3:一般信用(14日), 4:一般信用(ファンド) |
| 8 | 注文数量 | ● | ● | ● | number | 注文する株数 |
| 9 | 価格区分 | ● | ● | - | number | 0:成行, 1:指値（逆指値注文時は不要） |
| 10 | 注文価格 | △ | △ | - | number | 指値の場合の価格 |
| 11 | 執行条件 | ● | ● | ● | number | 1:本日中, 2:今週中, 3:寄付, 4:引け, 5:期間指定, 6:大引不成, 7:不成 |
| 12 | 注文期日 | △ | △ | △ | string | 期間指定時のYYYYMMDD形式日付 |
| 13 | 口座区分 | ● | ● | ● | number | 0:特定, 1:一般 |
| 14 | 逆指値条件価格 | - | ● | ● | number | 逆指値の条件価格 |
| 15 | 逆指値条件区分 | - | ● | ● | number | 1:以上, 2:以下 |
| 16 | 逆指値価格区分 | - | ● | ● | number | 0:成行, 1:指値 |
| 17 | 逆指値価格 | - | ● | ● | number | 逆指値の価格 |
| 18 | セット注文区分 | △ | △ | △ | number | 0:通常, 1:セット注文 |
| 19 | セット注文価格区分 | △ | △ | △ | number | 1:指値, 2:対価指定 |
| 20 | セット注文価格 | △ | △ | △ | number | セット注文の価格 |
| 21 | セット注文執行条件 | △ | △ | △ | number | セット注文の執行条件 |
| 22 | セット注文期日 | △ | △ | △ | string | セット注文の期日 |
